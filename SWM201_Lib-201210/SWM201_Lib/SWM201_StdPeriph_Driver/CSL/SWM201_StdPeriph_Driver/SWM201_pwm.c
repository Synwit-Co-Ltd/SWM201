...
* 注意事项: 无
******************************************************************************************************************************************/
void PWM_BrkConfig(PWM_TypeDef * PWMx, uint32_t chn, uint32_t brk, uint32_t out, uint32_t outN, uint32_t outHold)
{
	if(chn == PWM_CH_A)
	{
		...
		PWMx->BRKCR |= (out     << PWM_BRKCR_OUTA_Pos)  |
					   ...
					   (0       << PWM_BRKCR_STPCNT_Pos);	// 刹车状态下停止计数
	}
	else
	{
