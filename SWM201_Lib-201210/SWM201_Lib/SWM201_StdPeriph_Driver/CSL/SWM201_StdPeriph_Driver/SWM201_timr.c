...
* 函数名称: TIMR_Init()
* 功能说明:	TIMR定时器/计数器初始化
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，有效值包括TIMR0、TIMR1、BTIMR0、BTIMR1、BTIMR2、BTIMR3
*			uint32_t mode			对于TIMR0~3: TIMR_MODE_TIMER、TIMR_MODE_COUNTER、TIMR_MODE_OC、TIMR_MODE_IC
*									对于BTIMR0~3: TIMR_MODE_TIMER
*			uint32_t prediv			计数时钟预分频
*									对于TIMR0~3: 8位，取值1-256
*                       			对于BTIMR0~3: 16位，但大于256的分频值，必须能分解成两个小于等于256的数的乘积，比如分频值1024可以使用，因
*									为它可以分解成256*4，但1033不可以作为分频值，因为它不可以分解成两个整数的乘积
*			uint32_t period			定时/计数周期，取值1-65536
...
{	
	if((TIMRx == TIMR0) || (TIMRx == TIMR1))
	{
		...
		if(TIMR_SetPrediv(TIMRx, prediv) == 0)
			return;
...
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括TIMR0、TIMR1、BTIMR0、BTIMR1、BTIMR2、BTIMR3
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 函数名称:	TIMR_Halt()
* 功能说明:	暂停定时器，计数值保持不变
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括TIMR0、TIMR1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
*                       			对于BTIMR0~3: 16位，但大于256的分频值，必须能分解成两个小于等于256的数的乘积，比如分频值1024可以使用，因
*									为它可以分解成256*4，但1033不可以作为分频值，因为它不可以分解成两个整数的乘积
* 输    出: uint32_t				1 预分频设置成功   0 预分频设置失败
* 注意事项: 无
******************************************************************************************************************************************/
uint32_t TIMR_SetPrediv(TIMR_TypeDef * TIMRx, uint32_t prediv)
{
	uint16_t div1, div2 = 1;
	
	if((TIMRx == TIMR0) || (TIMRx == TIMR1))
	{
		if((prediv < 1) || (prediv > 256))
			return 0;
		
		TIMRx->CLKDIV = prediv - 1;
	}
	else// if((TIMRx == BTIMR0) || (TIMRx == BTIMR1) || (TIMRx == BTIMR2) || (TIMRx == BTIMR3))
	{
		for(div1 = 1; div1 < prediv; div1++)
		{
			if((prediv % div1) == 0)
			{
				div2 = prediv / div1;
				
				if((div1 <= 256) && (div2 <= 256))
					break;
			}
		}
...
* 功能说明:	获取定时/计数周期
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括TIMR0、TIMR1、BTIMR0、BTIMR1、BTIMR2、BTIMR3
* 输    出: uint32_t				定时/计数周期，取值1-65536
* 注意事项: 无
******************************************************************************************************************************************/
